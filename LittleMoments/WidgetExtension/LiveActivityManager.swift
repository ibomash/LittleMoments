...
final class LiveActivityManager {
  ...
  func startActivity(
    ...
    print(
      "🔄 Widget: Starting Live Activity - Session: \(sessionName), Target time: \(targetTimeInSeconds ?? 0) seconds"
    )
  ...
  func updateActivity(
    ...
    if Int(secondsElapsed) % 10 == 0 || isCompleted {
      print(
        "🔄 Widget: Updating Live Activity - Time: \(Int(secondsElapsed))s, Target: \(targetTimeInSeconds ?? 0)s, Completed: \(isCompleted)"
      )
    }
    ...
    let updatedState = MeditationLiveActivityAttributes.ContentState(
      ...
      showSeconds: activity.content.state.showSeconds,
