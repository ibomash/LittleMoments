...
final class WidgetBundleTests: XCTestCase {
  ...
  func testPreviewProviderUsesValidStateData() {
    ...
    if let targetTime = previewState.targetTimeInSeconds {
      XCTAssertLessThan(
        previewState.secondsElapsed, targetTime, "In-progress should be less than target")
    }

    if let targetTime = previewStateCompleted.targetTimeInSeconds {
      XCTAssertGreaterThanOrEqual(
        previewStateCompleted.secondsElapsed, targetTime, "Completed should be >= target")
    }
  }
  ...
  func testSharedTimerFunctionAccessibility() {
    ...
    XCTAssertEqual(
      timerDisplayFromSeconds(seconds: testSeconds, showSeconds: true), displayWithSeconds)
    XCTAssertEqual(
      timerDisplayFromSeconds(seconds: testSeconds, showSeconds: false), displayWithoutSeconds)
  }
