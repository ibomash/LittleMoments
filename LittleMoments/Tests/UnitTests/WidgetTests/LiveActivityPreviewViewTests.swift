...
final class LiveActivityPreviewViewTests: XCTestCase {
  ...
  func testPreviewProgressCalculation() {
    ...
    if let targetTime = timedState.targetTimeInSeconds {
      ...
      XCTAssertEqual(progress, 0.3, accuracy: 0.01, "Progress should be 30% for 3 minutes out of 10")
    } else {
    ...
    if let targetTime = completedState.targetTimeInSeconds {
      ...
      XCTAssertEqual(progress, 1.0, "Progress should be clamped to 100% even when elapsed exceeds target")
    } else {
